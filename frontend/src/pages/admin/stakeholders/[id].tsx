...
interface ChartPoint {
  label: string;
  earned: number | null;   // cumulative up through today (null for future points)
  scheduled: number;       // full projected cumulative (all points)
}
...
// Build full projected vesting schedule from grant parameters.
// Extends as far into the future as vesting continues.
function buildProjectedVestingData(grants: Grant[]): ProjectedData | null {
  ...
  // Accumulate every individual vest event across all grants
  ...
  for (const grant of optionGrants) {
    ...
    // Anchor at grant date so the chart history starts here
  ...
  // Insert a zero-delta anchor for today if no event falls in the current month,
  // so the "Today" ReferenceLine always lands on a real XAxis tick.
  ...
  // Extend the earned line to today by ensuring the last past point is included
  // (stepAfter will hold it flat up to the next future event)
...
export default function StakeholderDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/stakeholders/${id}/summary`, {
  ...
  const { stakeholder, balances, grants, vestingEvents } = summary;
    ...
    <>
      ...
      <div style={{ fontFamily: "'Outfit', sans-serif", minHeight: '100vh', background: '#0f172a', color: 'white' }}>
        ...
        <nav style={{ background: '#0f172a', borderBottom: '1px solid #1e293b', padding: '0 32px', height: '60px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <span style={{ fontWeight: 600, fontSize: '15px', color: 'white' }}>{stakeholder.name}</span>
        </nav>
              ...
              <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', padding: '24px 16px 16px' }}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={chartResult.points} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                    ...
                    {/* Full projected schedule — muted background */}
                    <Area
                      type="stepAfter"
                      dataKey="scheduled"
                      stroke="#475569"
                    ...
                    {/* Earned to date — solid amber foreground */}
                    <Area
                      type="stepAfter"
                      dataKey="earned"
                      stroke="#f59e0b"
