...
export default function NewGrant() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/company/${companyId}?success=grant`);
    } catch (err: unknown) {
          ...
          <form onSubmit={handleSubmit}>
              ...
              {!isNewEmployee && selectedStakeholder ? (
                ...
                <>
                  <div style={fieldRow}>
                    ...
                    <input required={isNewEmployee} value={form.employeeName} onChange={(e) => set('employeeName', e.target.value)} placeholder="e.g. Jane Smith" style={inputStyle} />
                  </div>
                  <div style={{ ...fieldRow, marginBottom: 0 }}>
                    <label style={labelStyle}>Email</label>
                    <input type="email" value={form.employeeEmail} onChange={(e) => set('employeeEmail', e.target.value)} placeholder="jane@company.com" style={inputStyle} />
                  </div>
                </>
              )}
            </section>
            ...
            <section style={sectionStyle}>
              ...
              <div style={fieldRow}>
                ...
                <input required value={form.securityName} onChange={(e) => set('securityName', e.target.value)} placeholder="e.g. Stock Options" style={inputStyle} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={fieldRow}>
                  <label style={labelStyle}>Number of Options *</label>
                  <input required type="number" min="0" step="any" value={form.quantity} onChange={(e) => set('quantity', e.target.value)} placeholder="e.g. 10000" style={inputStyle} />
                </div>
                <div style={fieldRow}>
                  <label style={labelStyle}>Strike Price (per share)</label>
                  <input type="number" min="0" step="any" value={form.strikePrice} onChange={(e) => set('strikePrice', e.target.value)} placeholder="e.g. 0.10" style={inputStyle} />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={fieldRow}>
                  <label style={labelStyle}>Grant Date *</label>
                  <input required type="date" value={form.grantDate} onChange={(e) => set('grantDate', e.target.value)} style={inputStyle} />
                </div>
                <div style={{ ...fieldRow, marginBottom: 0 }}>
                  <label style={labelStyle}>Board Approval Date</label>
                  <input type="date" value={form.boardApprovalDate} onChange={(e) => set('boardApprovalDate', e.target.value)} style={inputStyle} />
                </div>
              </div>
            </section>
            ...
            <section style={sectionStyle}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={fieldRow}>
                  ...
                  <input required type="number" min="0" step="1" value={form.cliffMonths} onChange={(e) => set('cliffMonths', e.target.value)} style={inputStyle} />
                </div>
                <div style={{ ...fieldRow, marginBottom: 0 }}>
                  <label style={labelStyle}>Total Duration (months) *</label>
                  <input required type="number" min="1" step="1" value={form.vestingDurationMonths} onChange={(e) => set('vestingDurationMonths', e.target.value)} style={inputStyle} />
                </div>
              </div>
              <div style={{ ...fieldRow, marginBottom: 0 }}>
                <label style={labelStyle}>Vesting Frequency *</label>
                <select value={form.vestingFrequency} onChange={(e) => set('vestingFrequency', e.target.value)} style={{ ...inputStyle, appearance: 'none' as const }}>
            ...
            <section style={sectionStyle}>
              ...
              <div style={fieldRow}>
                ...
                <input value={form.poolName} onChange={(e) => set('poolName', e.target.value)} placeholder="e.g. 2026 Equity Incentive Plan" style={inputStyle} />
              </div>
              {form.poolName.trim() && (
                <div style={{ ...fieldRow, marginBottom: 0 }}>
                  <label style={labelStyle}>Pool Authorized Shares</label>
                  <input type="number" min="0" step="any" value={form.poolShares} onChange={(e) => set('poolShares', e.target.value)} placeholder="e.g. 5000000" style={inputStyle} />
                </div>
              )}
            </section>
