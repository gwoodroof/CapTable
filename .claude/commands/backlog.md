...
| Arguments | Scope |
|---|---|
| `feature next` | First unchecked item in **FEATURES & ENHANCEMENTS** |
...
| `all` | All unchecked items in both sections — bugs first, then features |
...
6. **Mark complete in `Backlog.md`** — change `- [ ]` to `- [x]`, append a one-sentence implementation note, then move the entire entry to the **top** of the `# FRONTLOG` section.
